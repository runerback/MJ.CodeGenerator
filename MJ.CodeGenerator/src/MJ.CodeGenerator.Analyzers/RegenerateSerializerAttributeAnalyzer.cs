...
namespace MJ.CodeGenerator.Analyzers
{
    ...
    public class RegenerateSerializerAttributeAnalyzer : DiagnosticAnalyzer
    {
        public const string RuleId = "MJORLEANS0004";
        private const string Category = "Usage";
        private static readonly LocalizableString Title = new LocalizableResourceString(nameof(Resources.RegenerateSerializationAttributesTitle), Resources.ResourceManager, typeof(Resources));
        private static readonly LocalizableString MessageFormat = new LocalizableResourceString(nameof(Resources.RegenerateSerializationAttributesMessageFormat), Resources.ResourceManager, typeof(Resources));
        private static readonly LocalizableString Description = new LocalizableResourceString(nameof(Resources.RegenerateSerializationAttributesDescription), Resources.ResourceManager, typeof(Resources));
        ...
        private void CheckSyntaxNode(SyntaxNodeAnalysisContext context)
        {
            if (context.Node is TypeDeclarationSyntax declaration && !declaration.Modifiers.Any(m => m.IsKind(SyntaxKind.StaticKeyword)))
            {
                if (declaration.TryGetAttribute(Constants.GenerateSerializerAttributeName, out var attribute))
                {
                    ...
                    if (analysis.UnannotatedMembers.Count == 0 && analysis.DisorderedMembers.Count > 0)
                    {
